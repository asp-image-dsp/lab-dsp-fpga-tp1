...
#if !defined (LINT)
static char rcsid[] = "$Id: srec.c,v 1.44 1998/01/07 19:39:53 jay Exp $";	/* RCS data */
#endif
...
/*
  ...
  can find it.
*/
char Version[]   = "Version 6.2 ";	/* srec version number */
char Copyright[] = "(C) Copyright Motorola, Inc. 1987-1996.  All rights reserved.";

static char *optarg = NULL;	/* command argument pointer */
static int optind = 0;	/* command argument index */
...
static int dclflag = NO;			/* VMS DCL flag */
static int opts = NO;			/* single file option flag */
static int optm = NO;			/* multiple file option flag */
static int reverse = NO;			/* reverse byte option flag */
static int revlong = NO;			/* reverse word option flag */
static int baddr = NO;			/* byte address flag */
static int waddr = NO;			/* word address flag */
static int lmem = NO;			/* L memory flag */
static int quiet = NO;			/* signon banner flag */
static int xyflag = NO;			/* L data -> X/Y data */
static int trunc = NO;			/* truncate words to bytes */
...
static unsigned long wmask = 0L;	/* word mask */
static unsigned long wmaskp = 0L;	/* program word mask */
static unsigned long wmaskd = 0L;	/* data word mask */
static unsigned long amask = 0L;	/* address mask */
...
static char *memstr = "xylpedXYLPED";
static unsigned long memoff[] = {0L, 0L, 0L, 0L, 0L, 0L};
	...
	if (!dclflag) {		/* not using VMS DCL */
		...
		while ((c = getopts (argc, argv, "A:a:BbCcLlMmO:o:P:p:QqRrSsT:t:UuW?w?Xx")) != EOF) {
			...
			case 'm':
				...
				break;
			case 'o':
				if ((p = strchr (memstr, optarg[0])) == NULL
				    || optarg[1] != ':')
					usage ();
				if (isupper (*p))
					p -= MSPACES;
				if (!sscanf (&optarg[2], "%lx",
					     &memoff[p - memstr]))
					usage ();
				break;
			case 'p':
...
*/
	...
	if (lmem && xyflag)
		lmem = NO;	/* reset long memory address flag */
...
static int 
open_ifile (char *fn)	/* open input file, return file type */
{
	int ft;
	char *mode;
	char fname[MAXSTR];
	...
	(void)fix_fname (fn, ".cld");
	if ((ifile = fopen (fn, "rb")) != NULL)
		return (FT_CLD);
	(void)fix_fname (fn, ".lod");
	if ((ifile = fopen (fn, "r")) != NULL)
		return (FT_LOD);
	error2 ("cannot open input file %s", fname);
	return (FT_NONE);	/* not reached; for lint */
}


static void 
do_srec (FILE *fp)	/* determine type of object file */
{
	int c;
	...
	if (ftype == FT_CLD)
		...
		do_lod (fp);
	else {	/* try to guess type from file contents */
...
static void 
do_coff (FILE *fp)	/* process COFF file records */
{
	int i, spc;
	long nscns;
	...
	for (i = 1, sh = scn_header; i <= nscns; i++, sh++) {
		...
		read_rtn = sh->_s.s_flags & STYP_BLOCK ? read_bdata :
							 read_data;
		if (!(spc == LMEM && xyflag))
...
static void 
read_headers (FILE *fp)		/* read COFF file and optional headers */
{
	char *mstr = NULL;
	...
	if (file_header.f_opthdr)	/* optional header present */
		if (freads ((char *)&opt_header, (int)file_header.f_opthdr,
			    1, fp) != 1)
...
static void
read_data (XCNHDR *sh, int sn, int spc, int mem) /* read COFF section data */
{
	unsigned long *raw_data;
	int space = mem, i, mask = revlong ? !!mem : !mem;
	unsigned long addr, count = 0L;
	...
	emem_p = ((file_header.f_magic == M566MAGIC) && 
		  (sh->_s.s_flags & STYP_OVERLAYP));
	wsize = ((mem==PMEM) || (emem_p) )? wsizep : wsized;
	addr_incr = ((file_header.f_magic == M566MAGIC) && (mem==EMEM))?
	wsizee : wsize;
	if (!optm && baddr)
	...
	if ((raw_data = (unsigned long *)malloc ((unsigned)(sh->_s.s_size * sizeof (long)))) == NULL)
...
static void
read_bdata (XCNHDR *sh, int sn, int spc, int mem)
/* read COFF block section data */
{
	unsigned long raw_data[2];
	int space = mem, i, index = 0;
	unsigned long addr, repeat, count = 0L;
	unsigned long addr_incr;
	struct srec *drec;
	char snstr[MAXFLD];
	...
	emem_p = ((file_header.f_magic == M566MAGIC) && 
				 (sh->_s.s_flags & STYP_OVERLAYP));
	wsize = ((mem==PMEM) || (emem_p))? wsizep : wsized;
	addr_incr = ((file_header.f_magic == M566MAGIC) &&
		     (mem==EMEM))? wsizee : wsize;
	if (!optm && baddr)
		addr *= addr_incr;	/* adjust address for serial bytes */
	if (lmem && mem == LMEM && baddr)
		addr *= 2;	/* adjust address for long memory */
...
static void 
do_end (void)		/* generate S9 record(s) and clean up */
{
	int i, space;
	unsigned long count;	/* always a checksum byte */
	...
	for (space = 0; space < (opts ? 1 : MSPACES); space++) {
		if (rec[space]) {
			for (i = 0, drec = opts ? rec[0] : rec[space];
				...
				(void)sprintf (strbuf, s789fmt,
						count, fldbuf,
						~(checksum + count) & CSMASK);
...
static void 
do_lod (FILE *fp)	/* process load file records */
{
	int rtype, spc;
	long reset;
...
static int 
get_start (FILE *fp)	/* process START record */
{
	int type, old = NO;
	char *fbp = fldbuf;
	char *sbp = s0buf;
...
static int 
get_record (void)		/* look for next record in load file input */
{
	int field = 0;
	char *fbp = fldbuf + 1;
...
static void 
get_data (int spc, int mem)	/* process DATA records */
{
	int space = mem, i, mask = revlong ? !!mem : !mem;
...
*/
	...
	for (i = 0; get_field () == 0; i++) {	/* get next data field */
...
static void 
get_bdata (int spc, int mem)	/* process BLOCKDATA records */
{
	int space = spc = mem, i, j;
	unsigned long addr, val, count = 0L;
	unsigned long addr_incr;
	unsigned repeat;
	struct srec *drec;
...
static void 
get_end (void)		/* process END record and clean up */
{
	int i, field, space;
	...
	for (space = 0; space < (opts ? 1 : MSPACES); space++) {
		if (rec[space]) {
			...
			for (i = 0, drec = opts ? rec[0] : rec[space];
			     i < (optm ? wsize : 1); i++, drec++) {
				(void)sprintf (strbuf, s789fmt,	count,
					       field == 0 ? fldbuf : "",
						~(checksum + count) & CSMASK);
				if (fputs (strup (strbuf), drec->fp) == EOF)
					error ("cannot write end S-record");
...
static int 
sum_addr (unsigned long addr)	/* sum bytes of address */
{
	int i, sum;
...
static void 
flush_rec (int space, unsigned long addr, unsigned long count)
/* flush S1/S2/S3 record to appropriate file */
{
	...
	for (i = 0, drec = rec[space]; i < (optm ? wsize : 1); i++, drec++) {
		...
		(void)sprintf (strbuf, s123fmt, count, addr & amask, drec->buf,
			       ~(drec->checksum + count) & CSMASK);
		if (fputs (strup (strbuf), drec->fp) == EOF)
...
static void
check_addr (unsigned long addr)
/* reset S-record type based on address value */
{
	if (addr <= (unsigned long)MASK2) {
		if (asize != ASIZE1) {
			asize = ASIZE1;
			amask = MASK2;
			adrfmt = FMT4;
...
static void 
rev_bytes (char *buf,int mem)	/* reverse the bytes in buf */
{
	char c;
...
static void 
open_ofiles (int space)	/* open S-record ouput files */
{
	struct srec *drec;
	char fn[MAXSTR], *p = NULL;
	register i;
	char c;
...
static int 
get_field (void)	/* get next field from ifile; put in fldbuf */
{
	register c;
	register char *p;
...
static int 
get_comment (void)	/* get comment from load file; put in fldbuf */
{
	register c;
	register char *p;
...
static int 
get_line (void)		/* buffer line in strbuf */
{
	register char *p;
	register c;
...
static char *
scan_field (register char *bp) /* scan next field in strbuf; put in fldbuf */
{
	register char *p;
...
setup_mach (char *buf)
{
	...
	switch (alen) {		/* override address size settings */
	case ASIZE1:
		asize = alen;
		amask = MASK2;
		adrfmt = FMT4;
	...
	switch (tlen) {		/* override word size settings */
	case WSIZE2:
...
static int 
get_mem (enum memory_map mem)	/* return memory space from enumeration type */
{
	switch (mem) {
...
static int 
get_space (void)	/* return memory space attribute */
{
	switch (fldbuf[0]) {
	case 'X':
	case 'x':
...
static int 
get_memch (int spc)	/* return memory space character */
{
	switch (spc) {
	case XMEM:
		return ('x');
...
static int
get_ftype (char *fname)	/* try to determine file type by file extension */
{
	char *fn = basename (fname);
	char ext[MAXEXTLEN + 1], *ep;
	int i;
...
static char *
fix_fname (char *fn, char *ext)	/* add extension to file name if reqd */
{
...
static char *
strup (char *str)	/* convert all alpha chars in str to upper */
{
	register char *p = str;
...
static int 
fldhex (void)		/* insure fldbuf contains hex value */
{
...
static char *
basename (char *str)	/* return base part of file name in str */
{
	register char *p;
...
usage (void)			/* display usage on stderr, exit nonzero */
{
	...
	if (quiet)
		...
		(void) fprintf (stderr, "%s  %s\n%s\n",
				Progtitle, Version, Copyright);
	(void) fprintf (stderr, "Usage:  %s [-blmqrsuwx] [-a <alen>] [-o <mem>:<offset>] [-p <procno>] [-t <tlen>] <input file ... >\n", Progname);
